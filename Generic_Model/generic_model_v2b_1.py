...
def main():
    ...
    lt_ec = solph.Sink(label='LT-EC',
                       inputs={lt_wnw: solph.Flow(variable_costs=0)})
    ...
    if param['TES']['active']:
        ...
        for i in range(1, param['TES']['amount']+1):
            ...
            cost_Anlagen += (data_tes[(('LT-Wärmenetzwerk', label_id), 'flow')].sum()
            ...
            labeldict[(('LT-Wärmenetzwerk', label_id), 'flow')] = 'Q_zu_' + label_id
            labeldict[(('LT-Wärmenetzwerk', label_id), 'status')] = 'Status_zu_' + label_id
            labeldict[((label_id, 'None'), 'storage_content')] = 'Speicherstand_' + label_id

    # Heat exchanger
    labeldict[(('HNW_to_LT_HNW', 'LT-Wärmenetzwerk'), 'flow')] = 'Q_EX_LT-WNW'
    labeldict[(('Wärmenetzwerk', 'HNW_to_LT_HNW'), 'flow')] = 'Q_WNW_EX'
